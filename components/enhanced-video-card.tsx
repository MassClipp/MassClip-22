...
import { Heart, Download, Video, Music, ImageIcon, File } from "lucide-react"
...
export default function EnhancedVideoCard({
  ...
  const videoData = {
    ...
    mimeType: video.mimeType || video.fileType || "video/mp4",
  }

  // Determine content type
  const getContentType = (): "video" | "audio" | "image" | "document" => {
    const mimeType = videoData.mimeType.toLowerCase()
  ...
  const ContentIcon = () => {
    if (imageError) {
      if (contentType === "video") return <Video className="h-12 w-12 text-zinc-400" />
      if (contentType === "audio") return <Music className="h-12 w-12 text-zinc-400" />
      if (contentType === "image") return <ImageIcon className="h-12 w-12 text-zinc-400" />
  ...
  return (
    <div className="relative cursor-pointer" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <div className="relative aspect-[9/16] overflow-hidden rounded-lg bg-zinc-900">
        ...
        {!imageError ? (
          <img
            src={videoData.thumbnail || `/placeholder.svg?height=480&width=270&text=${contentType}`}
            alt={videoData.title}
          ...
          <div className="w-full h-full flex flex-col items-center justify-center">
            <ContentIcon />
            <p className="text-xs text-zinc-500 mt-2">{contentType}</p>
          </div>
        )}
