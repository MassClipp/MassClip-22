  ...
  return (
    <g className="recharts-cartesian-grid">
      ...
      {horizontalPoints?.map((point: number, index: number) => (
        <g key={`h-${index}`}>
          <line x1={x} y1={point} x2={x + width * 0.3} y2={point} stroke="rgba(71, 85, 105, 0.3)" strokeWidth="1" />
          <line
            x1={x + width * 0.35}
            y1={point}
            x2={x + width * 0.65}
      ...
      {verticalPoints?.map((point: number, index: number) => (
        <g key={`v-${index}`}>
          <line x1={point} y1={y} x2={point} y2={y + height * 0.3} stroke="rgba(71, 85, 105, 0.3)" strokeWidth="1" />
          <line
            x1={point}
            y1={y + height * 0.35}
            x2={point}
...
export function TechRevenueChart({ data }: TechRevenueChartProps) {
  ...
  if (!hasData) {
    return (
      <div className="h-64 relative bg-zinc-950/50 rounded-lg border border-zinc-800/50">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center">
            <div className="relative mb-6">
              <div className="w-20 h-20 border-2 border-cyan-400/40 transform rotate-45 mx-auto"></div>
              <div className="absolute inset-0 w-14 h-14 border-2 border-purple-400/40 transform rotate-45 m-auto"></div>
              <div className="absolute inset-0 w-8 h-8 border-2 border-blue-400/40 transform rotate-45 m-auto"></div>
              <div className="absolute inset-0 w-4 h-4 bg-cyan-400/60 transform rotate-45 m-auto animate-pulse"></div>
            </div>
            <p className="text-zinc-300 font-medium text-lg mb-2">Revenue Analytics Initializing</p>
  ...
  return (
    <div className="h-64 relative bg-zinc-950/50 rounded-lg border border-zinc-800/50 p-4">
