...
export default function UploadForm() {
  ...
  const getSignedUploadUrl = async (file: File) => {
    try {
      ...
      const username = user?.displayName?.replace(/\s+/g, "") || user?.email?.split("@")[0] || "anonymous"
  ...
  const goToProfile = () => {
    const username = user?.displayName?.replace(/\s+/g, "") || user?.email?.split("@")[0]
    if (username) {
      ...
      <div className="max-w-3xl mx-auto">
        <Card className="bg-black border-zinc-800 text-white">
          <CardHeader className="bg-gradient-to-r from-red-500/10 to-red-600/10 border-b border-red-500/20">
            ...
            <CardDescription className="text-zinc-400">Share your content with the world</CardDescription>
          </CardHeader>
