...
const PremiumContentSection: React.FC<PremiumContentSectionProps> = ({
  ...
  const runCheckoutDiagnostic = async (productBoxId: string) => {
    try {
      ...
      // Test 2: Basic Stripe connection
      console.log("🧪 [Diagnostic] Testing Stripe connection...")
      const connectionResponse = await fetch("/api/debug/stripe-connection-test")

      if (connectionResponse.ok) {
        const connectionData = await connectionResponse.json()
        console.log("✅ [Diagnostic] Stripe connection test passed:", connectionData)
      } else {
        const connectionError = await connectionResponse.json()
        console.error("❌ [Diagnostic] Stripe connection test failed:", connectionError)

        toast({
          title: "Stripe Connection Failed",
          description: connectionError.details || "Unable to connect to Stripe",
          variant: "destructive",
        })
        return
      }

      // Test 3: Direct product box test
      console.log("🧪 [Diagnostic] Testing direct product box access...")
      const productBoxResponse = await fetch(`/api/debug/product-box-test?id=${productBoxId}`)

      if (productBoxResponse.ok) {
        const productBoxData = await productBoxResponse.json()
        console.log("✅ [Diagnostic] Product box test result:", productBoxData)

        if (!productBoxData.success) {
          toast({
            title: "Product Box Access Failed",
            description: productBoxData.message || "Unable to access product box data",
            variant: "destructive",
          })
        }
      } else {
        console.error("❌ [Diagnostic] Product box test failed")
      }

      // Test 4: Full diagnostic
      console.log("🧪 [Diagnostic] Running full checkout diagnostic...")
      const response = await fetch("/api/debug/checkout-diagnostic", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (response.ok) {
        ...
        console.log("🔍 [Diagnostic] Full Results:", diagnostics)
        console.log("📊 [Diagnostic] Summary:", diagnostics.summary)

        // Show detailed diagnostic summary
        if (diagnostics.summary.issues.length > 0) {
          console.error("❌ [Diagnostic] Issues found:", diagnostics.summary.issues)

          toast({
            title: `Configuration Issues (${diagnostics.summary.failed} failed)`,
            description: `Issues: ${diagnostics.summary.issues.slice(0, 2).join(", ")}${diagnostics.summary.issues.length > 2 ? "..." : ""}`,
            variant: "destructive",
          })
        } else {
          ...
          toast({
            ...
            description: `All ${diagnostics.summary.passed} checks passed. Try the checkout again.`,
          })
        }
      }
    } catch (error) {
  ...
  const handlePurchase = async (productBox: ProductBox) => {
    try {
      ...
      console.log(`🛒 [Checkout] Product details:`, {
