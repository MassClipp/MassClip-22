...
export default function CreatorProfile({ creator }: { creator: Creator }) {
  ...
  return (
    <div className="relative z-10">
      ...
      {/* Creator Header */}
      <div className="relative z-10 mb-8 bg-zinc-900/30 backdrop-blur-sm border border-zinc-800/50 rounded-xl p-6 shadow-xl">
        <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
          <div className="w-24 h-24 md:w-32 md:h-32 rounded-full overflow-hidden bg-zinc-800 flex-shrink-0 border-2 border-crimson/30">
            {creator.profilePic ? (
              <img
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            {creator.bio && <p className="text-zinc-300 mt-4">{creator.bio}</p>}

            <div className="flex flex-wrap gap-2 mt-4 justify-center md:justify-start">
              <Button onClick={handleShare} variant="outline" className="border-zinc-700 text-white hover:bg-zinc-800">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="relative z-10">
        <TabsList className="grid w-full grid-cols-2 bg-zinc-900/50 backdrop-blur-sm border border-zinc-800/50 rounded-xl overflow-hidden">
          <TabsTrigger value="free" className="data-[state=active]:bg-crimson data-[state=active]:text-white py-3">
            Free Clips
          </TabsTrigger>
          <TabsTrigger value="premium" className="data-[state=active]:bg-crimson data-[state=active]:text-white py-3">
            Premium Clips
          </TabsTrigger>
        </TabsList>
          ...
          {creator.freeClips && creator.freeClips.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {creator.freeClips.map((clip, index) => (
                <Card key={index} className="bg-zinc-900/30 backdrop-blur-sm border-zinc-800/50 overflow-hidden">
                  ...
                  <CardContent className="p-4">
                    ...
                    {clip.description && <p className="text-zinc-400 mt-1 text-sm line-clamp-2">{clip.description}</p>}
                  </CardContent>
                  <CardFooter className="px-4 pb-4 pt-0 flex justify-between">
                    <Button size="sm" className="bg-crimson hover:bg-crimson/90">
                      Watch
                    </Button>
            ...
            <div className="text-center py-12 bg-zinc-900/30 backdrop-blur-sm rounded-xl border border-zinc-800/50">
              {isOwner ? (
          ...
          {creator.paidClips && creator.paidClips.length > 0 ? (
              ...
              {creator.paidClips.map((clip, index) => (
                <Card key={index} className="bg-zinc-900/30 backdrop-blur-sm border-zinc-800/50 overflow-hidden">
                  ...
                  <CardFooter className="px-4 pb-4 pt-0">
                    <Button className="w-full bg-crimson hover:bg-crimson/90">Unlock Content</Button>
                  </CardFooter>
                </Card>
              ))}
            </div>
            ...
            <div className="text-center py-12 bg-zinc-900/30 backdrop-blur-sm rounded-xl border border-zinc-800/50">
              {isOwner ? (
                ...
                <div className="space-y-4">
                  <Lock className="h-12 w-12 text-zinc-600 mx-auto mb-2" />
