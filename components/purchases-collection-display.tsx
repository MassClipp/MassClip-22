...
export default function PurchasesCollectionDisplay() {
  ...
  const fetchPurchases = async () => {
    try {
      setLoading(true)
      setError(null)
      console.log("🔄 [Purchases Collection] Starting fetch...")

      if (!user) {
        console.log("❌ [Purchases Collection] No user found")
        setError("Please log in to view your purchases")
        return
      }

      const token = await user.getIdToken()
      console.log("🔑 [Purchases Collection] Got auth token")
      ...
      console.log("📡 [Purchases Collection] API response status:", response.status)

      if (!response.ok) {
        if (response.status === 404) {
          console.log("ℹ️ [Purchases Collection] No purchases found (404)")
          setPurchases([])
          return
        }
        const errorText = await response.text()
        console.error("❌ [Purchases Collection] API error:", errorText)
        throw new Error(`Failed to fetch purchases: ${response.status} - ${errorText}`)
      }

      const data = await response.json()
      console.log("📦 [Purchases Collection] API response data:", data)

      // Handle case where API returns success but no purchases
      const purchasesArray = data.purchases || []
      console.log(`📊 [Purchases Collection] Processing ${purchasesArray.length} purchases`)
      ...
      console.log(`✅ [Purchases Collection] Loaded ${normalizedPurchases.length} purchases`)
    } catch (err) {
      console.error("❌ [Purchases Collection] Error fetching purchases:", err)
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-gray-300 text-base">
                {purchases.length} premium content items • ${purchases.reduce((sum, p) => sum + p.price, 0).toFixed(2)}{" "}
                total value
              </p>
            </div>
