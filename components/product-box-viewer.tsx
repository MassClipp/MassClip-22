...
interface ProductBoxViewerProps {
...
export default function ProductBoxViewer({
  productBox,
  isOpen,
  onClose,
  hasAccess = false,
  onPurchase,
}: ProductBoxViewerProps) {
  ...
  useEffect(() => {
    if (isOpen && productBox.contentItems.length > 0) {
      fetchVideos()
    }
  }, [isOpen, productBox.contentItems])

  const fetchVideos = async () => {
    try {
      setLoading(true)
      console.log(`🔍 [Product Box Viewer] Fetching ${productBox.contentItems.length} videos`)
      ...
      const videoPromises = productBox.contentItems.map(async (videoId) => {
        try {
          ...
          console.error(`❌ [Product Box Viewer] Error fetching video ${videoId}:`, error)
          return null
        }
      })
      ...
      console.log(`✅ [Product Box Viewer] Loaded ${fetchedVideos.length} videos`)
    } catch (error) {
      console.error("❌ [Product Box Viewer] Error fetching videos:", error)
      toast({
        title: "Error",
      ...
      <DialogContent className="bg-zinc-900 border-zinc-800 max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-start justify-between">
            <div className="space-y-2">
              <DialogTitle className="text-2xl text-white">{productBox.title}</DialogTitle>
              {productBox.description && <p className="text-zinc-400 text-sm max-w-2xl">{productBox.description}</p>}
            </div>
            <div className="flex items-center gap-2">
              <Badge variant="outline" className="border-amber-500 text-amber-400">
                {productBox.type === "subscription" ? "Subscription" : "One-time"}
              </Badge>
              <div className="text-right">
                ...
                {productBox.type === "subscription" && <span className="text-xs text-zinc-400">/month</span>}
              </div>
            </div>
          </div>
        </DialogHeader>
