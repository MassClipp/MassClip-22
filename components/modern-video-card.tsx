...
import { Download, Lock, Heart, Play } from "lucide-react"
...
export default function ModernVideoCard({ video }: ModernVideoCardProps) {
  ...
  if (!video) {
    return (
      <div className="flex-shrink-0 w-full">
        <div className="aspect-[9/16] rounded-lg bg-zinc-800/50 animate-pulse"></div>
        <div className="mt-2 h-4 bg-zinc-800/50 rounded animate-pulse"></div>
      </div>
    )
  }
  ...
  return (
    <div className="flex-shrink-0 w-full">
      <motion.div
        className="video-card video-card-hover relative"
        style={{
          aspectRatio: "9/16",
          overflow: "hidden",
        }}
      ...
      >
        ...
        <div
          className="absolute bottom-3 left-3 right-3 z-20 flex items-center justify-between transition-opacity duration-300"
          style={{ opacity: isHovered ? 1 : 0 }}
        >
          {/* Download button - visually disabled when limit reached */}
          <button
            className={`${
              hasReachedLimit ? "bg-zinc-800/90 cursor-not-allowed" : "bg-black/70 hover:bg-black/90 hover:scale-110"
            } p-2 rounded-full transition-all duration-300 ${downloadError ? "ring-1 ring-energy-500" : ""}`}
          ...
          >
            {hasReachedLimit ? (
              ...
              <Download className={`h-4 w-4 ${downloadError ? "text-energy-500" : "text-white"}`} />
            )}
          </button>

          {/* Favorite button */}
          <button
            className={`bg-black/70 hover:bg-black/90 hover:scale-110 p-2 rounded-full transition-all duration-300 ${
              isFavorite ? "text-energy-500" : "text-white"
            }`}
      ...
      <div
        ref={titleRef}
        className="mt-3 text-sm text-zinc-300 line-clamp-2 font-medium"
