...
import { RefreshCw, DollarSign, TrendingUp, CreditCard, Users, AlertCircle, CheckCircle, XCircle, Bug, Info, Loader2, Globe, Shield, ExternalLink } from 'lucide-react'
...
// Stripe Connection Component (matches the exact design from the screenshot)
function StripeConnectionSetup({ userId }: { userId: string }) {
...
export default function EarningsPage() {
  ...
  if (!stripeStatus?.connected || !stripeStatus?.chargesEnabled || !stripeStatus?.detailsSubmitted) {
    return <StripeConnectionSetup userId={user.uid} />
  }
