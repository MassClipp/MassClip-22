...
import { ArrowLeft, AlertCircle, Play, Package, Volume2, VolumeX, Pause, Download } from 'lucide-react'
...
const VideoPlayer = ({ content }: { content: BundleContent }) => {
  ...
  const handleDownload = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      link.download = `${content.title}.${content.fileType || 'mp4'}`
      link.target = '_blank'
      ...
      console.error(`❌ [VideoPlayer] Download error for ${content.title}:`, error)
      toast({
        title: "Download Error",
        description: `Failed to download: ${content.title}`,
        variant: "destructive",
      })
    } finally {
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="absolute bottom-3 right-3 w-8 h-8 bg-black/70 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-black/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
