...
export default function WelcomePage() {
  ...
  const handleStartTrial = async () => {
    try {
      ...
      if (!res.ok) {
        console.error("[Welcome] Failed to start trial")
        return
      }

      const data = await res.json()
      console.log("[Welcome] Trial started:", data)
        ...
        <div className="max-w-5xl w-full space-y-12">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="p-6 rounded-2xl bg-gradient-to-br from-zinc-900/50 to-black/50 border border-zinc-800/50 hover:border-cyan-400/30 transition-all">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h3 className="text-lg font-medium text-white">3-Day Free Trial</h3>
                  <p className="text-sm text-zinc-400">Try Creator Pro features</p>
                </div>
              </div>
              <div className="space-y-3 mb-6">
                {[
                  "Full Creator Pro access for 3 days",
                  "Unlimited folders & bundles",
                  "Full Vex AI access",
          ...
          <div className="text-center space-y-2">
            <p className="text-sm text-white/40">
              Free trial requires no credit card • Cancel anytime • Full access during trial
            </p>
          </div>
        </div>
      </div>
