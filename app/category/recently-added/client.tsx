...
// Avoid importing the entire useVimeoVideos hook directly
// Instead, create a simpler version that doesn't cause circular dependencies
export default function RecentlyAddedClient() {
  ...
  // Fetch videos directly instead of using the hook that might have circular dependencies
  useEffect(() => {
    async function fetchVideos() {
      try {
        // Simple fetch instead of using the hook
        const response = await fetch("/api/vimeo")
        if (!response.ok) throw new Error("Failed to fetch videos")
        const data = await response.json()

        // Process videos with dates
        const videosWithDates = data.map((video: any) => {
        ...
        const recent = videosWithDates.filter((video: any) => {
