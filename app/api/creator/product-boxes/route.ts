...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("🔍 [Product Boxes API] Fetching product boxes for user:", userId)

    // Use simple query without orderBy to avoid index requirements initially
    const snapshot = await db.collection("productBoxes").where("creatorId", "==", userId).get()

    // Sort in memory to avoid index requirements
    const productBoxes = snapshot.docs
      .map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }))
    ...
    console.log(`✅ [Product Boxes API] Found ${productBoxes.length} product boxes`)
    ...
    if (error instanceof Error) {
      console.error("❌ [Product Boxes API] Error details:", {
        name: error.name,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log("🔍 [Product Boxes API] Creating product box:", body.title)

    // Validate input data
    const validationError = validateProductBoxData(body)
    if (validationError) {
    ...
    console.log("🔍 [Product Boxes API] User data:", {
      userId,
    ...
    if (stripeAccountId) {
      console.log("🔍 [Product Boxes API] Attempting Stripe integration")

      // Validate Stripe account first
      const stripeValidationError = await validateStripeAccount(userData)
      if (stripeValidationError) {
        console.log("⚠️ [Product Boxes API] Stripe validation failed:", stripeValidationError.message)
        ...
        if (stripeResult.success) {
          ...
          console.log("✅ [Product Boxes API] Stripe integration successful")
        } else {
          console.log("⚠️ [Product Boxes API] Stripe integration failed:", stripeResult.error?.message)
    ...
    const docRef = await db.collection("productBoxes").add(productBoxData)
    ...
    console.log(`✅ [Product Boxes API] Created product box: ${docRef.id}`)
    ...
    const response = {
      ...
      message: productId
        ? "Product box created successfully and synced with Stripe"
        : `Product box created successfully${stripeError ? ` (Stripe sync failed: ${stripeError.message})` : " (Stripe sync skipped)"}`,
    }

    return NextResponse.json(response)
  } catch (error) {
    console.error("❌ [Product Boxes API] Error creating:", error)
    ...
    if (createdFirestoreDoc) {
      try {
        ...
        console.log("🔄 [Product Boxes API] Rolled back Firestore document")
      } catch (rollbackError) {
        console.error("❌ [Product Boxes API] Failed to rollback Firestore:", rollbackError)
      }
    }
...
function validateProductBoxData(data: any): ProductBoxCreationError | null {
  ...
  if (!title || typeof title !== "string" || title.trim().length === 0) {
    return {
      ...
      suggestedActions: ["Enter a descriptive title for your product box"],
    }
  }
  ...
  if (title.trim().length > 100) {
    return {
      ...
      suggestedActions: ["Shorten your product title to 100 characters or less"],
    }
  }
...
async function validateStripeAccount(userData: any): Promise<ProductBoxCreationError | null> {
  ...
  try {
    console.log("🔍 [Product Boxes API] Verifying Stripe account status")
    const account = await stripe.accounts.retrieve(userData.stripeAccountId)

    console.log("📊 [Product Boxes API] Stripe account details:", {
      id: account.id,
    ...
    console.log("✅ [Product Boxes API] Stripe account verified successfully")
    return null
  } catch (error: any) {
    console.error("❌ [Product Boxes API] Stripe account verification failed:", error)
    return {
...
function handleStripeError(error: any, context: "product" | "price"): ProductBoxCreationError {
  ...
  if (error.type === "StripeInvalidRequestError") {
    return {
      ...
      suggestedActions: [
        "Check that all product information is valid",
...
async function rollbackCreatedResources(
  ...
  try {
    console.log("🔄 [Product Boxes API] Rolling back created resources")

    // Rollback Firestore document
    if (firestoreDoc) {
      await firestoreDoc.delete()
      console.log("🔄 [Product Boxes API] Deleted Firestore document")
    }
    ...
    console.error("❌ [Product Boxes API] Failed to rollback resources:", error)
  }
}

async function rollbackStripeResources(productId: string, priceId: string, stripeAccountId: string): Promise<void> {
  try {
    console.log("🔄 [Product Boxes API] Rolling back Stripe resources")

    // Deactivate price first
    if (priceId) {
      await stripe.prices.update(priceId, { active: false }, { stripeAccount: stripeAccountId })
      console.log("🔄 [Product Boxes API] Deactivated Stripe price:", priceId)
    }

    // Then deactivate product
    if (productId) {
      await stripe.products.update(productId, { active: false }, { stripeAccount: stripeAccountId })
      console.log("🔄 [Product Boxes API] Deactivated Stripe product:", productId)
    }
  } catch (error) {
    console.error("❌ [Product Boxes API] Failed to rollback Stripe resources:", error)
  }
}
