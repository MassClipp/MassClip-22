...
// FACELESSPRENUER WEBHOOK - Only handles Facelessprenuer subscriptions
  ...
  try {
    ...
    switch (event.type) {
      ...
      case "customer.subscription.updated": {
        ...
        // Only process if this is a Facelessprenuer subscription
        if (!priceId || !FACELESSPRENUER_PRICE_IDS.includes(priceId)) {
          console.log(`[FACELESSPRENUER WEBHOOK] Ignoring non-Facelessprenuer subscription update: ${priceId}`)
          return NextResponse.json({ received: true })
        }
    ...
    console.error("[VIP/CREATOR PRO WEBHOOK] Error:", error)
