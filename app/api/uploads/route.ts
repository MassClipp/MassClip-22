...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileUrl, filename, title, size, mimeType } = body
    console.log("🔍 [Uploads API] Upload data:", { fileUrl, filename, title, size, mimeType })

    if (!fileUrl || !filename) {
      return NextResponse.json({ error: "Missing required fields: fileUrl, filename" }, { status: 400 })
    }
    ...
    const uploadData = {
      ...
      fileUrl,
      filename,
      title: title || filename.split(".")[0], // Use filename as default title
      type,
      size: size || 0,
