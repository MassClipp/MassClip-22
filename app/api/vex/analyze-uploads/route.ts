...
export async function POST(request: NextRequest) {
  try {
    ...
    let decodedToken
    try {
      console.log("[v0] Attempting to verify ID token...")
      decodedToken = await getAuth().verifyIdToken(token)
      console.log("[v0] Token verified successfully for user:", decodedToken.uid)
    } catch (authError: any) {
      console.error("[v0] Token verification error:", authError)
      console.error("[v0] Error type:", typeof authError)
      console.error("[v0] Error code:", authError.code)
      console.error("[v0] Error message:", authError.message)
      console.error("[v0] Token length:", token?.length)
      console.error("[v0] Token first 20 chars:", token?.substring(0, 20))
