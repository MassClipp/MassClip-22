...
${tierInfoData.reachedBundleLimit ? `⚠️ BUNDLE LIMIT REACHED: User has reached their limit of ${tierInfoData.bundlesLimit || 5} bundles. ${(tierInfoData.tier || "starter") === "starter" ? "They need to upgrade to Faceless Pro ($29/month) or Facelessprenuer ($39/month) for unlimited bundles." : "They should contact support."}` : ""}
`
            // </CHANGE>
  ...
  try {
    ...
    if (!subscriptionData.features.canCreateBundles) {
      return {
        success: false,
        error:
          "Bundle creation via Vex is a Faceless Pro ($29/month) or Facelessprenuer ($39/month) feature. Please upgrade your plan.",
      }
    }
    ...
    if (tierInfo.tier === "starter" && maxVideosPerBundle && contentIds.length > maxVideosPerBundle) {
      return {
        success: false,
        error: `Starter Plan users can only include up to ${maxVideosPerBundle} videos per bundle. This bundle has ${contentIds.length} items. Please upgrade to Faceless Pro ($29/month) or Facelessprenuer ($39/month) for unlimited videos per bundle.`,
      }
    }
