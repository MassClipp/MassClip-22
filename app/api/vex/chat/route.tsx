...
export async function POST(request: Request) {
  try {
    ...
    let analysisData: any = {} // Initialize analysisData
...
`
            ...
            if (analysisDoc.exists) {
              ...
              const analyzedAt = analysisData?.analyzedAt?.toDate?.() || analysisData?.lastUpdated?.()
              ...
              intelligenceContext +=
                "7. **READ TRANSCRIPTS FIRST** - If a video has a 'transcript' field, USE IT to understand the actual content. It's your primary source of truth.\n"
...
${analysisData?.detectedNiches?.length > 0 ? `\nDetected Content Niches: ${analysisData.detectedNiches.map((n: any) => `${n.name} (${n.count} items, ${n.avgConfidence}% avg confidence)`).join(", ")}` : ""}
${fileIdReferenceContext}${transcriptContext}${nicheContentsContext}${folderContext}`
...
Here is additional context about the user's content library, their plan, and their available features:
${userContentContext}
${folderContext}
${planPermissionsContext}
${bundleLimitsContext}
`,
    ...
    console.log("[v0] Sending messages to LLM:", JSON.stringify(messagesWithContext, null, 2))
    ...
    try {
      ...
      console.error("[v0] ❌ LLM processing error:", error)
